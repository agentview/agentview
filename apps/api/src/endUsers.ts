...
export async function createEndUser(values?: { externalId?: string | null, isShared?: boolean, simulatedBy?: string | null }) {
  const [newEndUser] = await db.insert(endUsers).values({
    externalId: values?.externalId ?? null,
    isShared: values?.isShared ?? false,
    simulatedBy: values?.simulatedBy ?? null,
...
export async function findEndUser({ id, externalId, token }: { id?: string, externalId?: string, token?: string }) {
  if (id) {

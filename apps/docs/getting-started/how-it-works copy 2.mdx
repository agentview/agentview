...
Even if your AI logic is the smartest in the world, your Chat Endpoint is not very practical yet. For example, it doesn't provide any persistence. So if you want to play with your agent, you'd have to build a front-end app and store history of conversation in `localStorage` and send entire conversation history back and forth between the client and the server. Of course, it's not something you want in production.
...
Both **backend** and **front-end** parts are not related to **AI Engineering**, they're stadnard boring engineering tasks. The job of AgentView is to handle them for you without contraining how you build your AI logic.

### Bulletproof your Chat Endpoint

AgentView provides 2 things:
- backend for conversational agents. It provides API to create, manage and access sessions, runs, items, users, etc.

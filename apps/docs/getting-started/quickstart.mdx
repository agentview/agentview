...
The example Agent Endpoint is in `agent.ts` file. Let's run it:
...
It's a simple Hono HTTP server exposing a single stateful AI endpoint `POST /simple_chat` and using OpenAPI Responses API to generate AI responses. Let's take a quick look:

...
In AgentView you provide AI logic as a HTTP endpoint. This gives you a full freedom over how you want to write AI and doesn't lock you in with any specific framework.
...
`server` is a location of AgentView Server and `agentApp` is a base URL of your stateless Agent Endpoint.
...
- created a session
- initiated the run
- called your Agent Endpoint to produce a response (`history` was empty)
...
As you can see AgentView took care of persistence and session, under the hood your Agent Endpoint got proper `history`.

...
To run AgentView server locally you need to install it via docker compose. There's a `docker-compose.yml` file in the root directory of the project. Just run:

```bash
docker compose up -d
```

Visit `http://localhost:1990/api/health` to see if the server is running. If you get 200 response with `{ "status": "ok", ... }` it means it's all good.

The API exposed by AgentView server is called **AgentView API**. It's used by Studio and your apps.

## Run Studio

Before we continue, let's run the studio:
...
Now go to the `localhost:1989` URL and register as the first admin user. You can also invite other users to the platform.

## Configure your first agent

At the root of the project there's a `agentview.config.tsx` file. All the configuration of AgentView is done via this file. It's used both by Studio and AgentView Server (Studio detects config file changes and synchronizes it with the Server).
...
With AgentView you can write your Agent Endpoint in any language or framework you want. The example Agent Endpoint is in `agent.ts` file. It's a simple Hono HTTP server computing AI responses using OpenAI Responses API.
...
2. `AGENTVIEW_API_KEY` - go to Studio, `Settings > API Keys` and grab your personal API key.
...
Your Agent Endpoint server should be running at `localhost:3000`. Let's take a quick look at the `agent.ts` file:

{ /* <prompt>
  copy apps/examples/typescript-basic/agent.ts file into following snippet
</prompt> */ }
...
You can already comment on the output, mention other team members, like/dislike the response, etc. You can also create a new Playground Session by clicking on "Create Session" at the top of the sidebar.
...
{ /* <prompt>
  - copy apps/examples/typescript-basic/agentview.config.tsx file into following snippet
  - leave only `export default defineConfig({...})` block
  - highlight `displayComponent` and `inputComponent` lines
</prompt> */ }

...
The following code says that `my_agent` has a single run.
...
By default, if AgentView can't find a run based on input item, it will fallback to "unknown run" which allows for any item shape. So this is still possible:
...
Any time an error occurs, AgentView will throw an exception which is an instance of `AgentViewError`. It has `body` and `statusCode` propreties so that it's trivial to just redirect it directly to the user as HTTP responses from your endpoint:

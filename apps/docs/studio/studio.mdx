...
The job of `inputComponent` is to submit a request to trigger a new run by making a request to your custom Agent Endpoint. Please keep in mind that *Studio doesn't listen to the response from your Agent*, as it might be a custom format. It displays live changes based on the data you sent to AgentView via `av.createRun` or `av.updateRun` calls.
...
| `submit` | A thin wrapper on top of native browser `fetch`, that manages state and simplifies code. | `(url: string, fetchOptions?: RequestInit & { input?: any }) => Promise` |
...
In your component you can make a call to your Agent Endpoint just by using standard browser `fetch` function. However it requires a bit of annoying boilerplate code to take into account cancellation, loading state etc.

`submit` is a very thin wrapper on top of `fetch` that handles those things:
- it sends request as `POST` and sets `Content-Type: application/json` header
...
Essentially, the second parameter of `submit` function is the same as you would pass to `fetch` function with extra `input` property. If you set `input`, the body will be `{ id, token, input: your_input }`.

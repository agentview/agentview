...
So far `my_super_agent` was very simple. Let's create a more advanced  example. We're gonna build `weather_agent`, it's bascially a very simple agent but capable of making a tool call that checks a weather in particular location.

To show the flexibility of AgnetView, let's give up on Responses API and use AgentsSDK. 

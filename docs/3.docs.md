...
app.post('/chat', async (c) => {
  const { sessionId, input, userToken } = await c.req.json();

  // create or get session
  const session = sessionId ?
    await av.getSession({ id: sessionId, userToken }):
    await av.createSession({ userToken })
    ...
    agents: [
        {
            ...
            inputComponent: ({ userToken, isLoading, cancel, submit }) => <UserMessageInput
                onSubmit={(value) => {

...
`AgentView` is a very thin wrapper to communicate with AgentView Server over AgentView API. 
...
The `apiKey` should be your **personal** key. Every developer working on AgentView should his/her own key. AgentView assigns every playground session to a specific author and the personal key is how it knows who's the session is.
